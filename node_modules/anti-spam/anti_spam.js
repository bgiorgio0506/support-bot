...
module.exports = function (bot, options) {
  ...
  const roleMessage = (options && options.roleMessage) || "has been banned for spamming, anyone else?";
  const maxDuplicatesWarning = (options && options.duplicates || 7);
  const maxDuplicatesBan = (options && options.duplicates || 10);
  const time = (options && options.time || 10);
  const roleID = (options && options.roleID || 10);
 ...
 function ban(msg, userid) {
   ...
   if (user) {
     ...
     var ticket_status = 'Non Definito';
     var sql  = "UPDATE assistenza SET ticket_status= ? WHERE sent_by_name =? AND ticket_status =?";
     var value =[status, msg.author.username, ticket_status];
         con.query(sql,value,function(err,res){
           if (err) throw err;
         console.log(res)
         });
     msg.member.addRole(role).then((member) => {
       msg.channel.send(msg.author + " " +roleMessage);
       console.log(`The user ${msg.author.username} was muted due to spam at ${new Date()}`);
                 var test = setTimeout(()=> {
                   console.log(`The user ${user} was unmuted at ${new Date()}`);
     user.removeRole(role)
}, time);
       return true;
    }).catch(() => {
       msg.channel.send("Has taken muted " + msg.author);
       return false;
    });
   }
 }
